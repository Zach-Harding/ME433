...
#define NUM_PIXELS 4
#define PWM_PIN 9 //Channel 4B of the PWM peripheral
...
int main() {
    ...
    for(int a=0;a<NUM_PIXELS;a++){
        wsColor c = HSBtoRGB(0.0, 1.0, 1.0);
        put_pixel(pio, sm, urgb_u32(c.r, c.g, c.b)); // assuming you've made arrays of colors to send
    }

    while (1) {
        for(int i = 0; i < 1000; i++) {
            float hue_base = (i / 1000.0) * 360.0;

            int j;
            for(j=0;j<NUM_PIXELS;j++){
                float hue_offset = hue_base + (j * 360.0 / NUM_PIXELS);
                wsColor c = HSBtoRGB((fmodf(hue_offset, 360.0f)), 1.0, 1.0);
                put_pixel(pio, sm, urgb_u32(c.r, c.g, c.b)); // assuming you've made arrays of colors to send
            }
            sleep_us(80);

            
            float angle = 1250 + (i/1000.0) * 5000;
            pwm_set_gpio_level(PWM_PIN, angle);
            sleep_ms(5); // 5s for loop
        }
    }
